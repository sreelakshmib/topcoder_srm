...
int GolfScore::tally(vector <int> p, vector <string> s) {
	int res=0;
	for(int i=0;i<p.size();i++){
		if(s[i]=="triple bogey")res+=p[i]+3;
		if(s[i]=="double bogey")res+=p[i]+2;
		if(s[i]=="bogey")res+=p[i]+1;
		if(s[i]=="par")res+=p[i];
		if(s[i]=="birdie")res+=p[i]-1;
		if(s[i]=="eagle")res+=p[i]-2;
		if(s[i]=="albatross")res+=p[i]-3;
		if(s[i]=="hole in one")res++;
	}
	return res;	
}
