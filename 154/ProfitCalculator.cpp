...
int ProfitCalculator::percent(vector <string> items) {
	...
	for(int i=0;i<items.size();i++){
		sell+=(items[i][0]-'0')*100+(items[i][1]-'0')*10+(items[i][2]-'0');
		sell+=double(double(items[i][4]-'0')*10+double(items[i][5]-'0'))/100;
		cost+=(items[i][7]-'0')*100+(items[i][8]-'0')*10+(items[i][9]-'0');
		cost+=double(double(items[i][11]-'0')*10+double(items[i][12]-'0'))/100;
	}
	profit=sell-cost;
	//cout<<sell<<" "<<cost<<" "<<profit;
	double ans=(profit*100)/sell;
	cout<<ans;
	return (int)ans;
