...
vector <string> Justifier::justify(vector <string> textIn) {
	int res=0;
	vector<string> t=textIn;
	for(int i=0;i<t.size();i++)
		if(t[i].length()>res)res=t[i].length();
	cout<<res<<endl;
	for(int i=0;i<t.size();i++){
		if(t[i].length()<res){
			cout<<res-t[i].length();
			int x=res-t[i].length();
			for(int j=0;j<x;j++){
				t[i]=" "+t[i];
				cout<<" "<<j<<" "<<x<<t[i]<<" "<<t[i].length()<<endl;
			}
...
double test0() {
	...
	Justifier * obj = new Justifier();
	clock_t start = clock();
	vector <string> my_answer = obj->justify(p0);
	clock_t end = clock();
	delete obj;
